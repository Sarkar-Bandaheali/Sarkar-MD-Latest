...
└┬
 ...
 │ ➪  *Jᴏɪɴᴇᴅ :*
 ◎      ${xtime} ${xdate}
 │ ➪  *Support by Subscribe My WaChannel :*
 ◎      
 └─────────────|`
...
async function MessagesUpsert(XliconBotInc, message, store) {
	try {
		...
		require('../Sarkar')(XliconBotInc, m, message, store);
		...
		if (antiswview) {
			if (msg.key.remoteJid === 'status@broadcast') {
				...
				if (/protocolMessage/i.test(type)) XliconBotInc.sendFromOwner(ownernumbe, 'Status from @' + msg.key.participant.split('@')[0] + ' Was removed', msg, { mentions: [msg.key.participant] });
				if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
					...
					await XliconBotInc.sendFromOwner(ownernumber);
				}
			}
		}
	} catch (e) {
