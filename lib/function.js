...
const GIFBufferToVideoBuffer = async (image) => {
    ...
    await fs.writeFileSync(`./bandahmedia/trash/${filename}.gif`, image)
    child_process.exec(
        `ffmpeg -i ./bandahmedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./bandahmedia/trash/${filename}.mp4`
    )
    await sleepy(4000)

    var buffer5 = await fs.readFileSync(`./bandahmedia/trash/${filename}.mp4`)
    Promise.all([unlink(`./bandahmedia/trash/${filename}.mp4`), unlink(`./bandahmedia/trash/${filename}.gif`)])
    return buffer5
}
